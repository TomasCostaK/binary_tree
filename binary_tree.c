...
/**
 * isert a node in the tree (it is assumed that the tree does not store repeated data)
...
{
  if(link == NULL) //If node is null
    return 0;
  // If there is no pointer to the left and no pointer to the right -> Leaf
  if (link->left == NULL && link->right == NULL)
    return 1;
  // If it's not a leaf:
  //    -> Increment the number of leaves to the left 
  //    -> After Increment the number of leaves to the right
...
int main(int argc,char **argv)
{
  ...
  FILE *fil = fopen("results.data","wb");
